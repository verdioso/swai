...
## Phase (Council Hotfix) — Title Interception & XML Hallucination Fixes

### What was fixed

1. **Bug A (Intercepted Title Requests):**
   - Expanded `is_auxiliary_request` in `core/src/proxy/council.rs` to detect `"write the title"` and `<session>` tags in user-role messages.
   - Claude Code's background requests to name the session tab now correctly bypass the Council pipeline instead of wasting ~60 seconds being audited as empty code implementations.

2. **Bug B (Planner XML Hallucination):**
   - Removed the `cli_system` prompt injection in `core/src/proxy/council_route.rs`.
   - Previously, Claude Code's system prompt containing `<tool_call>` XML examples was appended to the Planner's system prompt, causing the Planner to emit XML instead of the requested JSON schema. The Planner now relies solely on the strict JSON tool schema generated by `inject_tool_discipline_into_config`.

3. **Bug C (Missing Inspection Tools):**
   - Added `"explore"`, `"bash"`, `"bash_command"`, and `"execute_command"` to the whitelist in `is_immediate_inspection_directive` (`core/src/council/planner.rs`).
   - Planner directives requesting these read-only/exploration tools are now correctly executed immediately instead of being wrongly forwarded to the Generator for hallucinated implementation.

4. **Bug D (Malformed Fallback Tool Calls):**
   - Hardened the `extract_tool_call` write fallback in `core/src/proxy/tool_calling.rs`.
   - The generic whole-file-overwrite fallback is now strictly disabled for surgical tools (like Claude Code's `Edit` and `Replace` tools) that require missing parameters (e.g., `old_string`). This prevents the proxy from generating syntactically valid but structurally malformed tool calls that the client immediately rejects.
